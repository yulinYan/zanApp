<template>
  <view class="addbt">
    <view class="headersty">
      <u-navbar :custom-back="backPage" :title-color="'#323232'" :back-icon-color="'#323232'" :back-text="'返回'" :back-icon-name="'arrow-leftward'" :back-icon-size="34" :is-fixed="true" :is-back="true" :background="{background: '#fff'}" title="下架详情">
      ...
      <view class="listy">
        ...
        <u-table border-color="#dfdfdf">
          ...
          <u-tr class="u-tr" v-for="(item, index) in list" :key="item.id" :class="index % 2 === 0 ? 'fontsty' : 'fontsty linsty'">
            ...
            <u-td class="u-td">{{item.productName}}</u-td>
...
export default {
  ...
  mounted() {
    ...
    const maintainInfo = uni.getStorageSync('downShelfInfo')
    if (maintainInfo) {
      this.itemInfo = JSON.parse(maintainInfo)
      uni.removeStorageSync('downShelfInfo')
    }
    this.getList()
  },
  methods: {
    ...
    submit() {
      ...
      uni.$emit('reflashlistDown', {
        result: 1
      })
    },
    getList() {
      this.$u
        .get(this.apiUrl.getDownDetailList, {
          id: this.itemInfo.id
          // id: 6
        ...
        .then(res => {
          for (const item of res.data.logisticsDownHouseDetails) {
            this.list.push(item)
          }
