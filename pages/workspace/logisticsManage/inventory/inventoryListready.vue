      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          <u-col span="5" class="leftsty" @click="clickGet(item)">
            ...
            <view class="u-font-23 u-text-center u-margin-bottom-16 namesty">盘库单号</view>
            <view class="u-font-23 u-text-center fontsty u-margin-bottom-26 u-line-1">{{ item.houseNo }}</view>
          </u-col>
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            <view class="itemsty">
              盘点日期：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;"> {{ item.executeTime }} </view>
            </view>
            <view class="itemsty">
              盘点人：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.checkUserName }}</view>
            </view>
            <view class="itemstys">
              盘点仓库：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.checkHouseNames }}</view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.findCheckList, {
          houseNo: this.maintForm.searchKey,
          status: 1,
    ...
    clickGet(item) {
      console.log(item)
      uni.setStorageSync('inventoryInfo', JSON.stringify(item))
      this.$u.route({
        url: '/pages/workspace/logisticsManage/inventory/addForms'
      })
    }
  }
}
