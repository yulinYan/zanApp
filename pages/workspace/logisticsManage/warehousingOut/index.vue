<template>
  <view class="worksty">
    <view class="headersty">
      <u-navbar :title-color="'#323232'" :back-icon-color="'#323232'" :back-text="'返回'" :back-icon-name="'arrow-leftward'" :back-icon-size="34" :is-fixed="true" :is-back="true" :background="{background: '#fff'}" title="待入库任务">
    ...
    <view>
     ...
     <view class="bodys">
        <WarehousingOutList v-if="active === 1" />
        <WarehousingOutListready v-if="active === 2" />
      </view>
    </view>
...
<script>
import WarehousingOutList from './warehousingOutList'
import WarehousingOutListready from './warehousingOutListready'
import btns from '../../../components/btn'
export default {
  components: {
    btns,
	WarehousingOutList,
	WarehousingOutListready
  },
  data() {
    return {
      active: 1
  ...
  onLoad() {
    var self = this
    uni.$on('showWarehousingOutForm', function (data) {
