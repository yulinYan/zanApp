      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          <u-col span="5" class="leftsty" @click="clickGet(item)">
            ...
            <view class="u-font-23 u-text-center u-margin-bottom-16 namesty">移库单号</view>
            <view class="u-font-23 u-text-center fontsty u-margin-bottom-26 u-line-1">{{ item.moveHouseNo }}</view>
          </u-col>
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            ...
            <view class="itemstys">
              备注：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.note }}</view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.findMoveList, {
          moveHouseNo: this.maintForm.searchKey,
          status: 0,
    ...
    clickGet(item) {
      console.log(item)
      uni.setStorageSync('moveInfo', JSON.stringify(item))
      let self = this
      uni.$once('reflashmoveslist', function (data) {
        if (data.result === 1) {
          self.pageIndex = 1
          self.list = []
          self.getList()
      ...
      this.$u.route({
        url: '/pages/workspace/logisticsManage/moves/addForms'
      })
    }
  }
}
