      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          ...
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            <view class="itemsty">
              源单单号：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.orderNo }}</view>
            </view>
            <view class="itemsty">
              源单类型：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.kind === 0 ? '其他' :(item.kind === 1?'采购':'委外加工') }}</view>
            </view>
            <view class="itemstys">
              收货日期：
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.findTakeList, {
          taskNo: this.maintForm.searchKey,
          status: 0,
    ...
    clickGet(item) {
      console.log(item)
      uni.setStorageSync('takeInfo', JSON.stringify(item))
      let self = this
      uni.$once('reflashConfirmslist', function (data) {
        if (data.result === 1) {
          self.pageIndex = 1
          self.list = []
          self.getList()
      ...
      this.$u.route({
        url: '/pages/workspace/logisticsManage/confirms/addForms'
      })
    }
  }
}
