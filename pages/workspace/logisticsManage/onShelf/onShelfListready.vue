...
export default {
  ...
  methods: {
    ...
    clickGet(item) {
      uni.setStorageSync('onShelfInfo', JSON.stringify(item))
      let self = this
      uni.$once('reflashlistOnshelf', function (data) {
        //回调方法 用于刷新页面
        if (data.result === 1) {
          self.pageIndex = 1
          self.list = []
      ...
      this.$u.route({
        url: 'pages/workspace/logisticsManage/onShelf/addForms'
      })
    }
  }
}
