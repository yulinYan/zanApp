  ...
  <view class="maintainlis u-padding-25">
    <u-form :model="maintForm" ref="uForm">
      <u-form-item :leftIconStyle="{color: '#969696', fontSize: '23rpx'}" left-icon="search" label-width="25" label-position="left" label="" class="searchsty">
        <u-input v-model="maintForm.searchKey" placeholder-style="line-height: 63rpx;color: #969696;fontSize: 23rpx" placeholder="请输入订单号" type="text" />
      ...
      <view v-for="(item, index) of list" :key="index" class="listy">
        <u-row>
          ...
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            <view class="itemsty">
              订单号：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.houseNo }}</view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  data() {
    return {
      ...
      list: [
        // { keepOrder: '上模', mouldCode: '10P0001', status: '0' },
        // { keepOrder: '上模', mouldCode: '10P0001', status: '1' },
        // { keepOrder: '上模', mouldCode: '10P0001', status: '0' },
        // { keepOrder: '上模', mouldCode: '10P0001', status: '1' }
      ],
      pageIndex: 1,
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.findHistoryList, {
     ...
     */
    clickGet(item) {
      uni.setStorageSync('outsidesListreadyInfo', JSON.stringify(item))
      let self = this
      ...
      this.$u.route({
        url: 'pages/workspace/logisticsManage/outsides/addForms'
      })
    }
  }
}
