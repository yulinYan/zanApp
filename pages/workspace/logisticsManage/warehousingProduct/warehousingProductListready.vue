  ...
  <view class="maintainlis u-padding-25">
    <u-form :model="maintForm" ref="uForm">
      <u-form-item label-width="25" label-position="left" label="" class="searchsty">
        <u-search v-model="maintForm.houseNo" shape="square" placeholder="输入任务单号搜索" type="text" confirm-type="搜索" :show-action="false" @search="handleConfirmSearch"></u-search>
      </u-form-item>
    </u-form>
    <scroll-view :scroll-top="0" scroll-y="true" class="contsty" @scrolltolower="getBottom">
      <view v-for="(item, index) of list" :key="index" class="listy">
        <u-row>
          ...
          <u-col span="7" class="u-padding-left-26 rightsty" @click="clickGet(item)">
            <view class="itemsty">
              任务单号：
...
<script>
export default {
  name: 'WarehousingProductListready', //生产入库--已完成任务
  data() {
    return {
      searchPopup: false,
      maintForm: {
  ...
  methods: {
    ...
    clickGet(item) {
      this.$u.route({
        url: '/pages/workspace/logisticsManage/warehousingProduct/formsDetail'
      })
      uni.setStorageSync('warehousingProduct', JSON.stringify(item))
    }
  }
}
</script>
