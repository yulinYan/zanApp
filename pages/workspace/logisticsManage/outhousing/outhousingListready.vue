  ...
  <view class="maintainlis u-padding-25">
    <u-form :model="maintForm" ref="uForm">
      <u-form-item label-width="25" label-position="left" label="" class="searchsty">
        <u-search v-model="maintForm.houseNo" shape="square" placeholder="输入出库单号搜索" type="text" confirm-type="搜索" :show-action="false" @search="handleConfirmSearch"></u-search>
      </u-form-item>
    </u-form>
      ...
      <view v-for="(item, index) of list" :key="index" class="listy">
        <u-row>
          ...
          <u-col class=" u-padding-left-26 rightsty" span="7" @click="clickGet(item)">
            ...
            <view class="itemstys">
              出库类型：
              <view class="valuesty u-line-1" v-if="item.houseType*1 === 0">销售出库</view>
              <view class="valuesty u-line-1" v-if="item.houseType*1 === 1">生产出库</view>
              <view class="valuesty u-line-1" v-if="item.houseType*1 === 2">委外生产出库</view>
              <view class="valuesty u-line-1" v-if="item.houseType*1 === 3">其他出库</view>
            </view>
          </u-col>
        </u-row>
...
<script>
export default {
  name: 'OuthousingListready', //出库--已出库任务
  data() {
    return {
      searchPopup: false,
      maintForm: {
  ...
  methods: {
    ...
    getList() {
      let submitData = {
        houseNo: this.maintForm.houseNo.toString().trim(),
    ...
    clickGet(item) {
      this.$u.route({
        url: '/pages/workspace/logisticsManage/outhousing/formsDetail'
      })
      uni.setStorageSync('outhousing', JSON.stringify(item))
    }
  }
}
</script>
