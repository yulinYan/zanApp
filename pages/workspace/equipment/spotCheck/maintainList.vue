      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          <u-col span="4" class="leftsty" @click="clickGet(item)">
            ...
            <view class="u-font-23 u-text-center fontsty u-margin-bottom-26">NO.{{item.productDevices}}</view>
          </u-col>
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="8" @click="clickGet(item)">
            <view class="itemsty">
              点检工单号：
              <view class="valuesty">{{item.code}}</view>
            </view>
            <view class="itemsty">
              生产指令：
...
export default {
  ...
  methods: {
    ...
    clickGet(item) {
      ...
      //   result: 'sdf'
      // })
      uni.setStorageSync('spotCheckInfo', JSON.stringify(item))
      let self = this
      ...
      this.$u.route({
        url: '/pages/workspace/equipment/spotCheck/addForms'
      })
    },
    getList() {
      this.$u
        .get(this.apiUrl.getSpotUnifinshedTask, {
