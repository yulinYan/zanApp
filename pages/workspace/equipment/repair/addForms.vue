  ...
  <view class="addform u-padding-25">
    ...
    <view class="fromsifno">
      <u-form v-if="current === 0" :model="maintForm" ref="uForm">
        <u-form-item :label-style="{fontSize: '23rpx'}" :label-position="'left'" :border-bottom="false" label-width="169" label="设备编号" class="from-item scansty">
          <u-input v-model="maintForm.matchNumber" placeholder="扫描设备编号" placeholder-style="line-height: 63rpx;color: #969696;fontSize: 23rpx;paddingLeft: 25rpx;paddingRight: 75rpx;width: calc(100% - 100rpx)" maxlength="30" @blur="checkNumber()" />
          <u-icon name="scan" size="28" color="#006fe6" class="u-abso iconsty" @click="scan()" />
        </u-form-item>
        <u-form-item :label-style="{fontSize: '23rpx'}" :label-position="'left'" :border-bottom="false" label-width="169" label="设备类型" class="from-item">
          <u-input v-model="maintForm.typeName" :disabled="true" type="select" placeholder-style="line-height: 63rpx;color: #969696;fontSize: 23rpx;paddingLeft: 25rpx;paddingRight: 25rpx;width: calc(100% - 50rpx)" placeholder="请选择设备类型" />
        </u-form-item>
      ...
      <u-form v-if="current === 2" :model="maintForm" ref="uForm">
        ...
        <u-form-item :label-style="{fontSize: '23rpx'}" :label-position="'left'" :border-bottom="false" label-width="169" label="备品备件" class="from-uploaditem">
          <view style="width: 100%">
            <u-row v-for="(item, index) of sparePart" :key="index" class="u-margin-bottom-15">
              <u-col :span="12" class="u-flex">
                <view style="width: 170rpx;margin-right:5rpx">
                ...
                <u-radio-group v-model="item.status" :size="22" class="getRadio" @change="radioGroupChanges()">
...
export default {
  ...
  methods: {
    ...
    scan() {
      let self = this
      uni.setStorageSync('scan_type', 'getNumberByRepair')
      uni.$once('getNumberByRepair', function (data) {
        for (const item of self.codeList) {
          if (item.name === data.result) {
            self.maintForm.matchNumber = item.name
            self.maintForm.id = item.id
            self.maintForm.typeName = item.classifyName
            self.maintForm.type = item.classifyId
            break
          }
        }
      ...
      this.$u.route({
        ...
        params: {
          type: 'getNumberByRepair'
        }
      })
    },
    checkNumber() {
      for (const item of this.codeList) {
        if (item.name === this.maintForm.matchNumber) {
          this.maintForm.matchNumber = item.name
          this.maintForm.id = item.id
          this.maintForm.typeName = item.classifyName
          this.maintForm.type = item.classifyId
          break
        }
      }
    ...
    findDevicesLists() {
      this.$u.get(this.apiUrl.e2cUrl + this.apiUrl.deviceList, {}).then(res => {
    ...
    getStoreMachine() {
      this.$u.get(this.apiUrl.storeFindList, {}).then(res => {
        this.machineList = []
        ...
        .then(res => {
          if (res.code === 0) {
            ...
            if (res.data.list[0].repairProjectStatus === 0) {
              this.current = 1
              this.maintForm = {
                id: res.data.list[0].productDevicesId,
                matchNumber: res.data.list[0].devicesName,
                type: res.data.list[0].devicesClassify,
                typeName: res.data.list[0].classifyName,
              ...
              this.current = 2
              this.sparePart = []
              if (res.data.list[0].listSpare) {
                for (const item of res.data.list[0].listSpare) {
                  this.sparePart.push({
                    machineValue: item.devicesSpareId,
                    machineName: item.devicesSpareName,
              ...
              this.maintForm = {
                ...
                repariItemName: res.data.list[0].devicesRepairItemName,
            ...
            } else if (res.data.list[0].repairTaskExeStatus === 1 && res.data.list[0].taskReceived === 1) {
              this.current = 4
    ...
    submit() {
      if (this.current === 0) {
        ...
        this.$u
          .post(
            this.apiUrl.saveRepari,
            JSON.stringify({
              productDevicesId: this.maintForm.id,
              question: this.maintForm.question.join(','),
              questionDesc: this.maintForm.questiondesc,
              devicesClassify: this.maintForm.type,
        ...
        for (const item of this.sparePart) {
          if ((item.machineValue + '').length > 0) {
            list.push({
              devicesRepairId: this.currentId,
              devicesSpareId: item.machineValue,
