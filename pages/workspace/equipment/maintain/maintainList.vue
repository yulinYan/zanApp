      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          <u-col span="5" class="leftsty" @click="clickGet(item)">
            ...
            <view class="u-font-23 u-text-center fontsty u-margin-bottom-26 u-line-1">{{ item.keepOrder }}</view>
          </u-col>
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            <view class="itemsty">
              设备编号：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.devicesName }}</view>
            </view>
            <view class="itemsty">
              保养类型：
              <view class="valuesty">定期保养</view>
            </view>
            <view class="itemstys">
              工单状态：
              <view class="valuesty">{{ item.status === '0' ? '未执行' : (item.status === '1' ? '延期' : (item.status === '2' ? '强制关闭' : (item.status === '3' ? '已执行' : '执行中'))) }}</view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.keepTaskList, {
          keepOrder: this.maintForm.searchKey,
          taskStatus: '0,1,4',
    ...
    clickGet(item) {
      console.log(item)
      uni.setStorageSync('maintainInfo', JSON.stringify(item))
      let self = this
      uni.$once('reflashmaintainlist', function (data) {
        if (data.result === 1) {
          self.pageIndex = 1
          self.list = []
          self.getList()
      ...
      this.$u.route({
        url: '/pages/workspace/equipment/maintain/addForms'
      })
    }
  }
}
