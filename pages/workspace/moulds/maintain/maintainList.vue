      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          ...
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            ...
            <view class="itemsty">
              保养类型：
              <view class="valuesty">{{ item.keepType === 1 ? '定期保养' : (item.keepType === 2 ? '产前保养' : (item.keepType === 3 ? '在线保养' : '产后保养')) }}</view>
            </view>
            <view class="itemstys">
              工单状态：
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.mouldKeepTaskList, {
    ...
    clickGet(item) {
      ...
      let self = this
      uni.$once('reflashmouldsMainlist', function (data) {
        if (data.result === 1) {
          self.pageIndex = 1
          self.list = []
          self.getList()
      ...
      this.$u.route({
        url: '/pages/workspace/moulds/maintain/addForms'
      })
    }
  }
}
