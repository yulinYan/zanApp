      ...
      <view v-for="(item, index) of list" :key="index" class="listy" @click="clickGet(item)">
        <u-row>
          ...
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            ...
            <view class="itemstys">
              任务状态：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;" v-if="item.taskStatus === 0 && item.opreate == 0 "> 待上模 </view>
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;" v-if="item.taskStatus === 0 && item.opreate == 1 "> 待下模 </view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.mouldunUsedsList, {
          ...
          status: 0, //0 是未完成任务列表    1 是已完成任务列表
    ...
    clickGet(item) {
      uni.setStorageSync('mouldUsedsInfo', JSON.stringify(item))
      let self = this
      ...
      this.$u.route({
        url: '/pages/workspace/moulds/useds/addForms'
      })
    }
  }
}
