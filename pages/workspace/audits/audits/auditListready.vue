  ...
  <view class="maintainlis u-padding-25">
    <u-form :model="maintForm" ref="uForm">
      <u-form-item :leftIconStyle="{color: '#969696', fontSize: '23rpx'}" left-icon="search" label-width="25" label-position="left" label="" class="searchsty">
        <u-input v-model="maintForm.searchKey" placeholder-style="line-height: 63rpx;color: #969696;fontSize: 23rpx" placeholder="请输入任务单号" type="text" />
      ...
      <view v-for="(item, index) of list" :key="index" class="listy">
        <u-row>
          ...
          <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="7" @click="clickGet(item)">
            <view class="itemsty">
              表单名称：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.teamName }}</view>
            </view>
            <view class="itemsty">
              审核人员：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.userName }}</view>
            </view>
            <view class="itemstys">
              执行时间：
              <view class="valuesty u-line-1" style="width: 235rpx;line-height: 21rpx;">{{ item.exeTime }}</view>
            </view>
          </u-col>
        </u-row>
        <div class="subscript">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.getAuditList, {
          name: this.maintForm.searchKey,
          status: 1,
    ...
    clickGet(item) {
      console.log(item)
      uni.setStorageSync('auditInfo', JSON.stringify(item))
      this.$u.route({
        url: '/pages/workspace/audits/audits/addForms'
      })
    }
  }
}
