      ...
      <view v-if="!isDetail">
        <view v-for="(item, index) of list" :key="index" class="listy">
          <u-row @click="findDetail(item)">
            <u-col class="u-padding-top-26 u-padding-bottom-26 u-padding-left-26" span="8" @click="findDetail(item)">
              <view class="itemsty" @click="findDetail(item)">
                接收人：
          ...
          <div class="subscript" @click="findDetail(item)">
            完成
          </div>
        </view>
        <u-loadmore :status="status" />
      </view>
      <readyDetail v-if="isDetail"></readyDetail>
    </scroll-view>
  </view>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.$u
        .get(this.apiUrl.getShiftFinshedList, {
    ...
    findDetail(item) {
      uni.setStorageSync('shiftDetailInfo', JSON.stringify(item))
      this.isDetail = true
    },
    getBottom(e) {
